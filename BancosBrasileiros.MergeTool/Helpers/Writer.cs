...
internal static class Writer
{
    ...
    private static void SaveSql(IEnumerable<Bank> banks)
    {
        ...
        lines.AddRange(
            banks.Select(bank =>
                $"{prefix}'{bank.Compe:000}','{bank.Ispb:00000000}','{bank.Document}','{bank.LongName.Replace("'", "''")}','{bank.ShortName}',{(string.IsNullOrWhiteSpace(bank.Network) ? "NULL" : $"'{bank.Network}'")},{(string.IsNullOrWhiteSpace(bank.Type) ? "NULL" : $"'{bank.Type}'")},{(string.IsNullOrWhiteSpace(bank.PixType) ? "NULL" : $"'{bank.PixType}'")},{(string.IsNullOrWhiteSpace(bank.ChargeStr) || !bank.Charge.HasValue ? "NULL" : $"{(bank.Charge.Value ? 1 : 0)}")},{(string.IsNullOrWhiteSpace(bank.CreditDocumentStr) || !bank.CreditDocument.HasValue ? "NULL" : $"{(bank.CreditDocument.Value ? 1 : 0)}")},{(bank.LegalCheque ? 1 : 0)},{(bank.DetectaFlow ? 1 : 0)},{(string.IsNullOrWhiteSpace(bank.PcrStr) || !bank.Pcr.HasValue ? "NULL" : $"{(bank.Pcr.Value ? 1 : 0)}")},{(string.IsNullOrWhiteSpace(bank.PcrpStr) || !bank.Pcrp.HasValue ? "NULL" : $"{(bank.Pcrp.Value ? 1 : 0)}")},{(string.IsNullOrWhiteSpace(bank.SalaryPortability) ? "NULL" : $"'{bank.SalaryPortability}'")},{(bank.Products == null ? "NULL" : $"'{string.Join(",", bank.Products)}'")},{(string.IsNullOrWhiteSpace(bank.Url) ? "NULL" : $"'{bank.Url}'")},{(string.IsNullOrWhiteSpace(bank.LogoUrl) ? "NULL" : $"'{bank.LogoUrl.Replace("'", "''")}'")},{(string.IsNullOrWhiteSpace(bank.DateOperationStarted) ? "NULL" : $"'{bank.DateOperationStarted}'")},{(string.IsNullOrWhiteSpace(bank.DatePixStarted) ? "NULL" : $"'{bank.DatePixStarted}'")},'{bank.DateRegistered:O}','{bank.DateUpdated:O}');"
            )
        );
